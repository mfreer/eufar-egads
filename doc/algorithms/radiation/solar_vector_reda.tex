...
\item Calculate the topocentric zenith angle (in degrees):
  \begin{enumerate}
    ...
    \item Calculate the topocentric elevation angle without atmospheric correction (in radians):
    \begin{displaymath}
    e_0 = \sin^{-1} (\sin{lat} \sin{\delta'} + \cos{lat} \cos{\delta'} \cos{H'})
    \end{displaymath}
    ...
    Note that this step is skipped if temperature and pressure are not provided by the user.

    \item Calculate the topocentric elevation angle (in degrees):
    \begin{displaymath}
    e = \frac{180}{\pi} e_0 + \Delta e
    \end{displaymath}
